from sudoku import valid_cell, solve, find_empty_cell
        ...
        for i in range(9):
            for j in range(9):
                if random.randint(1, 10) >= 5:
                    ...
                    if valid_cell(board, (i, j), board[i][j]):
                        continue
                    else:
                        board[i][j] = 0
...
class Board:
    ...
    def visualSolve(self, wrong, time):
        ...
        for nums in range(9):
            if valid_cell(self.board, (empty[0], empty[1]), nums + 1):
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("SUDOKU AI GAME")
